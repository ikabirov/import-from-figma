...
type Typography = Pick<TypeStyle, 'italic' | 'fontWeight' | 'fontSize' | 'lineHeightPx' | 'fontFamily'> & { name: string }
...
async function parseColors(pageId: string): Promise<ColorData[] | undefined> {
    ...
    const colors: ColorData[] = nodes.map((node) => node?.document as Rectangle).map(node => ({
        name: node.name,
