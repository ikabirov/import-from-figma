...
async function loadNodes(ids) {
  const {
    data
  } = await client.fileNodes(documentId, {
    ids
  });
...
function loadSvgUrls(ids) {
  ...
  return new Promise(async resolve => {
    ...
    while (ids.length) {
      ...
      const {
        data
      } = await client.fileImages(documentId, {
...
async function parseTypography(pageId) {
  const node = await loadNode(pageId);

  if (!(node != null && node.styles)) {
    return;
  }
  ...
  const textNodes = (await loadNodes(textKeys)).map(v => v.document).map(v => _extends({
    name: v.name
  }, v.style));
  return textNodes;
}

async function parseColors(pageId) {
  const node = await loadNode(pageId);
  ...
  const colors = nodes.map(node => node == null ? void 0 : node.document).map(node => ({
    name: node.name,
...
function writeFile(path$1, content) {
  shelljs.mkdir('-p', path.dirname(path$1));
...
function formatComponentName(name) {
  const words = name.replace(/[^a-z0-9]+/gi, ' ').trim().split(' ');
...
async function importFromFigma(config) {
  if (config.exportType == 'react') initializeReactResource(config); // if (config.exportType == 'flutter') initializeFlutterResource(config)
